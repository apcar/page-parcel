...
export function getMetrics(): Metrics {
  ...
  if (scroller) {
    ...
    return {
      ...
      initialScrollY: scroller.scrollTop,
      container: {
        x: r.left,
        y: r.top,
...
export function prepareCapture(): void {
  ...
  root.dataset.ccpScrollBehavior = root.style.getPropertyValue('scroll-behavior');
  root.dataset.ccpScrollBehaviorPriority = root.style.getPropertyPriority('scroll-behavior');
  document.documentElement.style.setProperty('scroll-behavior', 'auto', 'important');
  const scroller = document.querySelector<HTMLElement>('[data-oss-scroller="1"]');
  if (scroller) {
    scroller.dataset.ccpScrollBehavior = scroller.style.getPropertyValue('scroll-behavior');
    scroller.dataset.ccpScrollBehaviorPriority =
      scroller.style.getPropertyPriority('scroll-behavior');
...
export function hideFixedElements(): void {
  ...
  for (const el of els) {
    ...
    if (cs.position === 'fixed' || cs.position === 'sticky') {
      ...
      (el as HTMLElement).dataset.ccpVisibility = (el as HTMLElement).style.getPropertyValue(
        'visibility',
      );
      (el as HTMLElement).dataset.ccpVisibilityPriority = (
        el as HTMLElement
...
export function restoreCapture(initialScrollY = 0): void {
  const hidden = document.querySelectorAll('[data-oss-hidden="1"]');
  for (const el of hidden) {
    const htmlEl = el as HTMLElement;
    const value = htmlEl.dataset.ccpVisibility || '';
    const priority = htmlEl.dataset.ccpVisibilityPriority || '';
    if (value) htmlEl.style.setProperty('visibility', value, priority);
    else htmlEl.style.removeProperty('visibility');
    delete htmlEl.dataset.ossHidden;
    delete htmlEl.dataset.ccpVisibility;
    delete htmlEl.dataset.ccpVisibilityPriority;
  }
  const scroller = document.querySelector<HTMLElement>('[data-oss-scroller="1"]');
  if (scroller) {
    const value = scroller.dataset.ccpScrollBehavior || '';
    const priority = scroller.dataset.ccpScrollBehaviorPriority || '';
    if (value) scroller.style.setProperty('scroll-behavior', value, priority);
    else scroller.style.removeProperty('scroll-behavior');
    scroller.scrollTop = initialScrollY;
    delete scroller.dataset.ossScroller;
    delete scroller.dataset.ccpScrollBehavior;
    delete scroller.dataset.ccpScrollBehaviorPriority;
  } else {
    window.scrollTo(0, initialScrollY);
  }
  const root = document.documentElement;
  const rootValue = root.dataset.ccpScrollBehavior || '';
  const rootPriority = root.dataset.ccpScrollBehaviorPriority || '';
  if (rootValue) root.style.setProperty('scroll-behavior', rootValue, rootPriority);
  else root.style.removeProperty('scroll-behavior');
  delete root.dataset.ccpScrollBehavior;
  delete root.dataset.ccpScrollBehaviorPriority;
}
